...
bigint bigint_zero() {
    ...
    result.size = 1;
    result.digits = kmalloc(sizeof(int64_t));
    result.digits[0] = 0;
    return result;
}
...
void bigint_remove_leading_zeros(bigint *n) {
    ...
    // If there are leading zeros, kmemmove the digits
    if (leading_zeros > 0) {
        ...
        kmemmove(n->digits, n->digits + leading_zeros, n->size * sizeof(int64_t));
    }
}
...
bigint bigint_from_int(int64_t n) {
    ...
    result.digits = kmalloc(result.size * sizeof(int64_t));
    m = n;
...
bigint bigint_add(bigint a, bigint b) {
    ...
    if (carry > 0) {
        result.size++;
        result.digits = krealalloc(result.digits, result.size * sizeof(int64_t));
