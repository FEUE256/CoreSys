...
EFI_FILE_PROTOCOL *esp_root_dir(VOID) {
    ...
    if (EFI_ERROR(status)) {
        error(status, u"0x020200000000010000010001000000010000000000000000000000000101\r\n");
        goto done;
    }
    ...
    status = bs->OpenProtocol(lip->DeviceHandle,
                              ...
                              EFI_OPEN_PROTOCOL_GET_PROTOCOL);
    if (EFI_ERROR(status)) {
        error(status, u"0x020200000000010000010001000000010000000000000000000000000201\r\n");
        goto done;
    }
    ...
    if (EFI_ERROR(status)) {
        error(status, u"0x020200000000010000010001000000010000000000000000000000000301\r\n");
    }
...
VOID *read_esp_file_to_buffer(CHAR16 *path, UINTN *file_size) {
    ...
    *file_size = 0;
    ...
    root = esp_root_dir();
    if (!root) {
        error(0, u"0x020200000000010000010001000003020000000000000000000000000401\r\n");
        goto cleanup;
    }
    ...
    if (EFI_ERROR(status)) {
        error(status, u"0x020100000000010000010001000003020000000000000000000000000101 '%s'\r\n", path);
        goto cleanup;
    }
    ...
    if (EFI_ERROR(status)) {
        error(status, u"0x020300000000010000010001000003020000000000000000000000000101 '%s'\r\n", path);
        goto cleanup;
    }
    ...
    if (EFI_ERROR(status) || buf_size != file_info.FileSize) {
        error(status, u"0x020100000000010000010001000003020000000000000000000000000201 '%s'\r\n", path);
        goto cleanup;
    }
    ...
    if (EFI_ERROR(status) || buf_size != file_info.FileSize) {
        error(status, u"0x020100000000010000010001000003020000000000000000000000000301 '%s'\r\n", path);
        goto cleanup;
    }
...
read_disk_lbas_to_buffer(EFI_LBA disk_lba, UINTN data_size, UINT32 disk_mediaID, bool executable) {
    ...
    if (EFI_ERROR(status)) {
        error(status, u"0x020300000000010000010001060503040000000000000000000000000201\r\n");
        goto done;
    }
    ...
    if (!found) {
        error(0, u"0x020300000000010000010001060503040000000000000000000000000301 %u.\r\n", disk_mediaID);
        goto done;
    }
    ...
    status = bs->OpenProtocol(handle_buffer[i], 
                              ...
                              EFI_OPEN_PROTOCOL_GET_PROTOCOL);
    if (EFI_ERROR(status)) {
        error(status, u"0x020200000000010000010001060503040000000000000000000000000501 %u.\r\n", i);
        goto done;
    }
    ...
    status = bs->AllocatePages(AllocateAnyPages, 
                               ...
                               &buffer);
    if (EFI_ERROR(status)) {
        error(status, u"0x020100000000010000010001060503040000000000000000000000000401\r\n");
        goto done;
    }
    ...
    if (EFI_ERROR(status)) 
        error(status, u"0x020300000000010000010001060503040000000000000000000000000401\r\n");
...
EFI_STATUS set_largest_gop_mode(EFI_GRAPHICS_OUTPUT_PROTOCOL **ret_gop) {
    ...
    if (EFI_ERROR(status)) {
        error(status, u"0x020300000000010000010001000000070000000000000000000000000501\r\n");
        return status;
    }
    ...
    if (EFI_ERROR(status)) {
        error(status, u"0x020200000000010000010001000000070000000000000000000000000601 %u.\r\n", gop->Mode->Mode);
        return status;
    }
    ...
    if (!found_mode) 
        ...
        if (EFI_ERROR(status)) {
            error(status, u"0x020300000000010000010001000000070000000000000000000000000601 %u.\r\n", largest_res_mode);
            return status;
        }
...
EFI_STATUS set_gop_mode(EFI_GRAPHICS_OUTPUT_PROTOCOL **ret_gop, UINT32 xres, UINT32 yres) {
    ...
    status = bs->LocateProtocol(&gop_guid, NULL, (VOID **)&gop);
    if (EFI_ERROR(status)) {
        error(status, u"0x020300000000010000010001000505070000000000000000000000000501\r\n");
        return status;
    }
    ...
    status = gop->QueryMode(gop, gop->Mode->Mode, &mode_info_size, &mode_info);
    if (EFI_ERROR(status)) {
        error(status, u"0x020200000000010000010001000505070000000000000000000000000601 %u.\r\n", gop->Mode->Mode);
        return status;
    }
    ...
    if (!found_mode) 
        ...
        if (EFI_ERROR(status)) {
            error(status, u"0x020300000000010000010001000505070000000000000000000000000601 %u.\r\n", save_mode);
            return status;
        }
...
EFI_STATUS check_gop_mode(UINT32 *ret_mode, UINT32 xres, UINT32 yres) {
    ...
    status = bs->LocateProtocol(&gop_guid, NULL, (VOID **)&gop);
    if (EFI_ERROR(status)) {
        error(status, u"0x020300000000010000010001000505050000000000000000000000000501\r\n");
        return status;
    }
    ...
    status = gop->QueryMode(gop, gop->Mode->Mode, &mode_info_size, &mode_info);
    if (EFI_ERROR(status)) {
        error(status, u"0x020200000000010000010001000505050000000000000000000000000601 %u.\r\n", gop->Mode->Mode);
        return status;
    }
    ...
    if (!found_mode)
        ...
        status = gop->QueryMode(gop, save_mode, &mode_info_size, &mode_info);
        if (EFI_ERROR(status)) {
            error(status, u"0x020300000000010000010001000505050000000000000000000000000601 %u\r\n", save_mode);
            return status;
        }
    }
...
EFI_STATUS get_disk_image_mediaID(UINT32 *mediaID) {
    ...
    status = bs->OpenProtocol(image,
                              ...
                              EFI_OPEN_PROTOCOL_GET_PROTOCOL);
    if (EFI_ERROR(status)) {
        error(status, u"0x020200000000010000010001000000050000000000000000000000000101\r\n");
        goto done;
    }
    ...
    status = bs->OpenProtocol(lip->DeviceHandle,
                              ...
                              EFI_OPEN_PROTOCOL_GET_PROTOCOL);
    if (EFI_ERROR(status)) {
        error(status, u"0x020100000000010000010001000000050000000000000000000000000501\r\n");
        goto done;
    }
...
//   using information found in the SEVI.TXT file in the ESP,
...
VOID *read_data_partition_file_to_buffer(char *in_name, bool executable, UINTN *ret_size) {
    ...
    if (EFI_ERROR(status)) {
        error(status, u"0x020300000000010000010001000306080000000000000000000000000701\r\n");
        goto cleanup;
    }

    // Get SEVI.TXT file from path "/EFI/BOOT/SEVI.TXT"
    CHAR16 *file_name = u"\\EFI\\BOOT\\SEVI.TXT";
    UINTN buf_size = 0;
    esp_file = read_esp_file_to_buffer(file_name, &buf_size);
    if (!esp_file) {
        error(0, u"0x020100000000010000010001000306080000000000000000000000000701\r\n", file_name);
        goto cleanup;
    }

    // Get disk LBA and file size from SEVI.TXT for input file name 
    char *str_pos = stpstr(esp_file, in_name);
    if (!str_pos) {
        error(0, u"0x010200000000010000010001000306080000000000000000000000000101\r\n", in_name);
        get_key();
    ...
    if (!str_pos) {
        error(0, u"0x020200000000010000010001000306080000000000000000000000000701\r\n", in_name);
        goto cleanup;
    }
    ...
    if (!str_pos) {
        error(0, u"0x020300000000010000010001000306080000000000000000000000000801\r\n", in_name);
        goto cleanup;
    }
    ...
    *ret_size = file_size;
    if (!data_file) {
        error(0, u"0x010200000000010000010001000306080000000000000000000000000201\r\n", in_name);
        *ret_size = 0;
        get_key();
        status = EFI_SYSTEM_FAIL;
        sf(__FILE__, __LINE__, __func__, status, L"LBA FAIL\n", L"0x010200000000010000010001000306080000000000000000000000000201\n");
    } 
...
EFI_HII_PACKAGE_LIST_HEADER *hii_database_package_list(UINT8 package_type) {
    ...
    if (EFI_ERROR(status)) {
        error(status, u"0x020200000000010000010001000000090000000000000000000000000801\r\n");
        goto cleanup;
    }
    ...
    if (status != EFI_BUFFER_TOO_SMALL && EFI_ERROR(status)) {
        error(status, u"0x020300000000010000010001000000090000000000000000000000000901\r\n",
              package_type);
        goto cleanup;
    }
    ...
    if (EFI_ERROR(status)) {
        error(status, u"0x020200000000010000010001000000090000000000000000000000000901\r\n",
              package_type);
        goto cleanup;
    }
    ...
    status = dbp->ListPackageLists(dbp, package_type, NULL, &buf_len, &handle_buf); 
    if (EFI_ERROR(status)) {
        error(status, u"0x020300000000010000010001000000090000000000000000000000000A01\r\n",
              package_type);
        goto cleanup;
    }
    ...
    if (status != EFI_BUFFER_TOO_SMALL && EFI_ERROR(status)) {
        error(status, u"0x020100000000010000010001000000090000000000000000000000000801\r\n",
              package_type);
        goto cleanup;
    }
    ...
    if (EFI_ERROR(status)) {
        error(status, u"0x020100000000010000010001000000090000000000000000000000000901\r\n",
              package_type);
        goto cleanup;
    }
    ...
    status = dbp->ExportPackageLists(dbp, handle, &buf_len2, pkg_list); 
    if (EFI_ERROR(status)) {
        error(status, u"0x020100000000010000010001000000090000000000000000000000000A01\r\n",
              package_type);
        goto cleanup;
    }
...
void *mmap_allocate_pages(Memory_Map_Info *mmap, UINTN pages) {
    ...
    if (remaining_pages < pages) {
        ...
        if (i >= mmap->size / mmap->desc_size) {
            ...
            error(0, u"\r\n0x02010000000001000001000100000A030000000000000000000000000B01\r\n");
            return NULL;
        }
    }
...
void set_runtime_address_map(Memory_Map_Info *mmap) {
    ...
    if (!runtime_mmap) {
        error(0, u"0x0202000000000100000100010000000A0000000000000000000000000A01\r\n");
        return;
    }
