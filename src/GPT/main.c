...
bool add_disk_image_info_file(FILE *image) {
    ...
    if (!opened_info_file) {
        ...
        fp = fopen("SEVI.TXT", "wb");  // Truncate on first open
    } else {
        fp = fopen("SEVI.TXT", "ab");  // Add to end of file
    }
    if (!fp) return false;
...
bool add_file_to_data_partition(char *filepath, FILE *image) {
    ...
    char info_file[12] = "SEVI.TXT"; // "Data (partition) files info"
...
Options get_opts(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (!strcmp(argv[i], "-e") || !strcmp(argv[i], "--efifilepath")) {
                if (i + 1 < argc) { // se till att det finns en fil efter -e
                    options.efi_file = argv[i + 1]; // spara filnamnet
                    i++; // hoppa över filnamnet i nästa iteration
                } else {
...
int main(int argc, char *argv[]) {
    ...
    if (options.help) {
        ...
        fprintf(stderr,
                ...
                "                       a <SEVI.TXT> file in directory '/EFI/BOOT/' in the \n"
                ...
                "-e  --efifilepath      The file path to the EFI file (BOOTX64.EFI).\n"
    ...
    //   add it to the ESP
    const char *efi_name = options.efi_file;  // pointer to string
    fp = fopen(efi_name, "rb");
    if (fp) {
        char path[256] = { 0 };  // make it bigger to avoid overflow
        snprintf(path, sizeof(path), "/EFI/BOOT/BOOTX64.EFI"); // dynamic path
        if (!add_path_to_esp(path, fp, image))
            fprintf(stderr, "Error: Could not add file '%s' as BOOTX64.EFI\n", path);
